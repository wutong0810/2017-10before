...
Mape=np.zeros((1,7))
Rmse=np.zeros((1,7))
gbm0 = RandomForestRegressor(n_estimators=200, min_samples_split=50,\
                                      min_samples_leaf=20,max_depth=9,max_features='sqrt',random_state=10)  
gbm0.fit(train_x,train_y)
for i in range(7):
    test_x=np.array(test_data2.iloc[144*i:144*(i+1),:-1])
    test_y=np.array(test_data2.iloc[144*i:144*(i+1),-1])
...
for i in range(7):
    plt.subplot(2,4,i+1)
    plt.plot(np.linspace(0,24,144),state.iloc[i*144:(i+1)*144,0],'o-',markersize=3,label='true') 
    plt.plot(np.linspace(0,24,144),state.iloc[i*144:(i+1)*144,1],'o-',markersize=3,label='predict')
    plt.legend()
    plt.ylim(0,700)
...
for i in range(7):
    plt.subplot(2,4,i+1)
    plt.plot(np.linspace(0,24,144),state.iloc[i*288:(i+1)*144,3],'o-',markersize=3,label='true') 
    plt.plot(np.linspace(0,24,144),state.iloc[i*288:(i+1)*144,4],'o-',markersize=3,label='predict')
    plt.legend()
    plt.ylim(0,4)

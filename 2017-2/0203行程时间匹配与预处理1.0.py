# -*- coding: utf-8 -*-
"""
Created on Thu Feb 23 09:21:33 2017
...
def deal_5min(match_time,num=3,length=575,day=0):
    match_time1=np.array(match_time)
    time_5min=np.zeros((289,7))
    time_5min[:,0]=range(289)
    time_5min[:,4]=(day+2)%7
    for i in range(288):
...
#   用历史数据补全
    ...
    a2=plt.figure()
    for i in range(15):
        a2=plt.subplot(5,3,i+1)
        a2=plt.plot(tt_deal_5min2[i][:,0],tt_deal_5min2[i][:,5],'o-')
##        a2=plt.plot(tt_deal_5min[i][:,0],tt_deal_5min[i][:,2],'o-')
#        plt.plot(med_deal[:,0],med_deal[:,1],'o-')
#        plt.plot(med_deal[:,0],med_deal[:,2],'o-')

    

#行程时间预测
#数据分析，利用历史数据学习，提取前三个数据作为后一个数据的预测内容；
    for i in range(31):
        med_data=tt_deal[i]
        med_deal=deal_5min(med_data,num=3,day=i)
        med_deal2=pd.DataFrame(med_deal)
